...
def ingest_source(path: Path, llm: LLMClient | None = None, workspace: Path | None = None) -> IngestResult:
    ...
    return _ingest_source(Path(path), client, settings.workspace)


def query_knowledge(question: str, workspace: Path | None = None, top_k: int = 5) -> QueryResult:
    settings = Settings.from_env(workspace)
    client = build_llm_client(settings)
    return _query_knowledge(question, client, settings.workspace, top_k=top_k)
